...
@testset "elementary polygons get merged" begin
    ...
    @test out.x == [3.0, 2.0, 1.0, 1.0, 2.0, 2.5, 3.0]
    @test out.y == [1.0, 1.0, 1.0, 1.5, 1.5, 2.0, 2.0]
    @test out.id == ones(7)
    ...
    z = [1 2 1;
         1 2 2]
    out = isobands(1:3, 2:-1:1, z, 0.5, 1.5)
    @test out.x == [3.0, 2.5, 3.0, 1.0, 1.5, 1.5, 1.0]
    @test out.y == [1.5, 2.0, 2.0, 2.0, 2.0, 1.0, 1.0]
    ...
    @test out.x == [3.0, 2.0, 1.0, 1.0, 1.0, 1.5, 1.5, 2.0, 2.5, 2.5, 3.0, 3.0]
    @test out.y == [1.50, 1.25, 1.50, 2.00, 3.00, 3.00, 2.00, 1.75, 2.00, 3.00, 3.00, 2.00]
    @test out.id == ones(12)
    ...
    out = isobands(1:3, 3:-1:1, z, 0.5, 1.5)
    @test out.x == [3.0, 2.0, 1.0, 1.0, 1.0, 2.0, 3.0, 3.0, 2.0, 2.5, 2.0, 1.5]
    @test out.y == [1.0, 1.0, 1.0, 2.0, 3.0, 3.0, 3.0, 2.0, 1.5, 2.0, 2.5, 2.0]
    @test Set(out.id) == Set([1, 2])
...
@testset "NaNs are handled correctly" begin
    ...
    out = isobands(1:3, 3:-1:1, z, 0.5, 1.5)
    @test out.x == [3, 2, 1, 1, 2, 2, 3, 3]
    @test out.y == [1, 1, 1, 2, 2, 3, 3, 2]
    @test out.id == ones(8)
    ...
    out = isobands(1:3, 3:-1:1, z, 0.5, 1.5)
    @test out.x == [1, 1, 2, 2, 2, 2, 3, 3]
    @test out.y == [1, 2, 2, 1, 2, 3, 3, 2]
    @test Set(out.id) == Set([1, 2])
...
@testset "Seven-sided saddles" begin
    ...
    @test out.x == [6.00, 6.00, 5.50, 5.00, 4.50, 4.00,
    ...
    @test out.y == [1.50, 1.00, 1.00, 1.25, 1.00, 1.00,
    ...
    @test Set(out.id) == Set(1:4)
    @test length(out.id) == 42
  
    # midpoint outside the band
    out = isobands(1:6, 3:-1:1, z, 0.8, 1.2)
    @test out.x == [6.0, 6.0, 5.8, 5.0, 4.4, 5.0,
    ...
    @test out.y == [1.2, 1.0, 1.0, 1.4, 2.0, 2.6,
...
@testset "Eight-sided saddles" begin
    ...
    @test out.x == [3.00, 3.00, 2.60, 2.75, 2.00,
        2.40, 2.25, 2.00, 1.75, 1.60,
        1.00, 1.00, 1.25, 1.40]
    @test out.y == [1.75, 1.60, 2.00, 2.00, 1.40,
    ...
    # midpoint inside the band
    out = isobands(1:3, 2:-1:1, z, 0.5, 1.5)
    @test out.x == [2.75, 2.25, 2.00, 1.75, 1.25,
        1.00, 1.00, 1.25, 1.75, 2.00,
        2.25, 2.75, 3.00, 3.00]
    @test out.y == [1.00, 1.00, 1.25, 1.00, 1.00,
    ...
    @test out.x == [3.00, 3.00, 2.75, 2.60, 2.40,
        2.00, 1.60, 1.75, 2.00, 2.25,
        1.40, 1.25, 1.00, 1.00]
    @test out.y == [1.40, 1.25, 1.00, 1.00, 2.00,
